...
export async function getGoogleAccessToken(
  ...
  try {
    ...
    for (const uid of userIds) {
      ...
      console.log(`[Google OAuth] Looking up token for user_id: ${uid}`);
      ...
      if (error) {
        console.error(`[Google OAuth] Database query error for user_id ${uid}:`, error);
      ...
      if (!tokenData) {
        console.warn(`[Google OAuth] No token found for user_id: ${uid}`);
        // Try next user ID
        continue;
      }

      console.log(`[Google OAuth] Token found for user_id: ${uid}, expires_at: ${tokenData.expires_at}`);
